...
# Step 0: 키워드 입력
if st.session_state.step == 0 or st.session_state.reset_triggered:
...
if st.session_state.step == 2:
    ...
    if topic == "자동이체" and sub == "가족 명의 계좌":
        st.markdown("- 예금주 신분증<br>- 계약자 신분증<br>- 가족관계증명서(3개월 이내)", unsafe_allow_html=True)
    elif topic == "타인카드" and sub == "구비서류":
        st.markdown("- 계약자 신분증<br>- 카드 소유주 신분증", unsafe_allow_html=True)
        ...
        st.markdown("- 계약자 신분증<br>- 수익자 신분증<br>- 증권 원본 제출 시 수익자 동의 생략 가능", unsafe_allow_html=True)
    else:
